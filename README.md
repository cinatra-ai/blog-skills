...
Editorial guidance for the Cinatra blog agents. This bundle ships the prompts behind blog idea brainstorming, full long-form drafting, and short LinkedIn promotion — keeping voice, structure, and source-handling rules consistent so generated posts stay on-brand for the target company and never leak the underlying source material.

Install this extension from the Cinatra marketplace and attach it to a WordPress or Drupal blog agent. The agent receives a transcript file and a target website URL; the skills guide it to extract relevant themes, draft a polished post that matches the site's editorial style, and produce a LinkedIn teaser with a canonical link. No transcript origin, speaker name, or third-party company name surfaces in the output.

Configuration is minimal: no credentials are required by the skills themselves. The host agent must already be connected to your CMS (WordPress or Drupal) through its own connector. To develop or extend a skill, edit the relevant SKILL.md under skills/, re-run the extension kind gate (node extension-kind-gate.mjs --package-root .), and open a pull request. If generated content repeats a transcript speaker's name, verify the agent is using the latest version of this bundle and that no older skill cache is active.
